      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-20 sm:py-24">
        ...
        <div className="mt-14 grid grid-cols-1 md:grid-cols-3 gap-6">
          {testimonials.map((item) => (
            <figure
              key={item.name}
              className="flex flex-col bg-white rounded-2xl border border-gray-100 shadow-sm p-8"
            >
              <div className="flex gap-0.5 text-emerald-500 mb-4" aria-label="5 out of 5 stars">
              ...
              <figcaption className="flex items-center gap-3">
                <span className="w-10 h-10 bg-emerald-100 rounded-full flex items-center justify-center text-emerald-700 font-semibold">
                  {item.name.charAt(0)}
                </span>
                <span>
